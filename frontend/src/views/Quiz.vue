<template>
  ...
  <v-card class="mx-auto" width="80%" min-width="300">
    ...
    <v-card-actions style="display: flex; justify-content: end">
      <v-btn style="color: #fff; background-color: #2d4356" @click="submitAnswer"> 정답제출 </v-btn>
    </v-card-actions>
  </v-card>
  </div>
</template>
...
const fetchQuiz = async () => {
  try {
    const { data } = await axios.get("http://localhost:8080/quizs/submit");
    quiz.value = {
      id: data[0].id,
...
const submitAnswer = async () => {
  ...
  try {
    const {data} = await axios({
      url: "http://localhost:8080/quizs/" + quiz.value.id + "/answer",
      method: "post",
      headers: {
